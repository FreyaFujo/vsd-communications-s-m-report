...
export const CompetitiveStrategyView: React.FC<CompetitiveStrategyViewProps> = ({ userProfile, deals, setDeals }) => {
  ...
  const handleUpdateNews = (id: string, news: string) => {
    setCompetitors(prev => prev.map(c => c.id === id ? { ...c, recentNews: news } : c));
  };
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="p-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium hidden md:block text-lg">
      ...
      <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 h-2/3 lg:h-full bg-slate-50 custom-scrollbar">
            <div className="max-w-6xl mx-auto space-y-10">
                {/* TOP SECTION: Market Intelligence (SWOT, News, Notes) */}
                <div className="bg-white p-8 rounded-3xl border-2 border-indigo-50 shadow-2xl flex flex-col relative overflow-hidden group">
                    ...
                    {/* Section 1: SWOT Display - ENHANCED READABILITY */}
                    <div className="flex flex-col gap-4 mb-8 relative z-10">
                        ...
                        <div className="p-6 bg-white rounded-xl border-2 border-slate-200 max-h-[400px] overflow-y-auto custom-scrollbar shadow-inner">
                            <div className="prose prose-slate prose-xl max-w-none text-slate-800 prose-headings:text-indigo-900 prose-headings:font-black prose-p:text-slate-800 prose-p:leading-loose prose-li:text-slate-800 prose-li:leading-loose prose-strong:text-indigo-700">
                                <MarkdownRenderer content={selectedCompetitor?.swotAnalysis || 'No analysis generated.'} />
                            </div>
                        </div>
                    </div>

                        {/* Section 2: Recent News Input */}
                    <div className="flex flex-col gap-3 mb-8 relative z-10 border-t border-indigo-100 pt-6">
                        ...
                        <textarea
                            className="w-full h-32 p-4 rounded-xl border-2 border-slate-200 bg-white text-slate-900 font-medium text-lg focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 outline-none shadow-sm resize-none leading-relaxed placeholder-slate-400"
                            placeholder="Track recent acquisitions, leadership changes, or public announcements..."
                            value={selectedCompetitor?.recentNews || ''}
                            onChange={(e) => selectedCompetitor && handleUpdateNews(selectedCompetitor.id, e.target.value)}
                        />
                    </div>

                    {/* Section 3: Notes & AI Generation - ENHANCED READABILITY */}
                    <div className="flex-1 flex flex-col relative z-10 border-t border-indigo-100 pt-6">
                            ...
                            <button 
                                ...
                                className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-md active:scale-95 disabled:opacity-50"
                            >
                                {suggestingNotesId === selectedCompId ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} className="text-amber-300" />}
                                Generate Strategy Notes
                            </button>
                            </div>

                        <textarea 
                            className="flex-1 w-full resize-none outline-none text-xl text-slate-900 font-medium placeholder-slate-400 p-6 rounded-2xl bg-white border-2 border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 shadow-sm leading-loose min-h-[250px]"
                ...
                {/* BOTTOM SECTION: Grid for Strategy and Deals */}
